...
def converter(Rdpath: str, method=""):
    """
    Take an .Rd file and write most of its informations in a dictionary.

    :param Rdpath: The file location of the .Rd file
    :param method: The method within Rd file to convert.
    :return a dictionary with the structure desired to dump into YAML.
    >>> path, name = os.path.split(__file__)
    >>> rd_path = os.path.join(path,'test-data', 'test.Rd')
    >>> data = converter(rd_path)
    >>> len(data)
    """
    with open(Rdpath, 'r') as Rdfile:
        ...
        if len(declaration_area) > 1:
            ...
            if method in declaration_name_list:
                choosen_temp = re.compile(r'\\method\{' + name + r'}\{' + method + '\}[^\\\]+')
